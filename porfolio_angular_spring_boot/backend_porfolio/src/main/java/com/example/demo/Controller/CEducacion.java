...
public class CEducacion {
    ...
    public ResponseEntity<?> create(@RequestBody dtoEducacion dtoeducacion){
        ...
        Educacion educacion = new Educacion(
                ...
                "https://firebasestorage.googleapis.com/v0/b/porfolio-jose-antille.appspot.com/o/imagen_generica%2Fnot_image.jpg?alt=media&token=28f701d8-7756-4213-a932-9c0e12d0558c", dtoeducacion.getAnioInicio(),
                dtoeducacion.getAnioFin(), dtoeducacion.getCarrera());
    ...
    @PutMapping("/updateImgEdu/{id}")
    public ResponseEntity<?> updateImgEdu(@PathVariable("id") int id, @RequestBody dtoEducacion dtoeducacion){
        /*if(!sEducacion.existsById(id)){
            return new ResponseEntity(new Mensaje("No existe el ID"), HttpStatus.NOT_FOUND);
        }
        if(sEducacion.existsByNombreE(dtoeducacion.getNombreE()) && sEducacion.getByNmbreE(dtoeducacion.getNombreE()).get().getId() != id){
